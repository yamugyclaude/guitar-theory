...
export function render(panel) {
  ...
  function renderAll() {
    ...
    panel.innerHTML = `
      ...
      <div class="section-label" style="margin:0 0 4px">시그니처 보캐뷸러리</div>
      <p style="font-size:0.74rem;color:var(--text2);margin:0 0 8px">이 연주자를 카피할 때 채집해야 할 어휘 목록 — 각 항목을 실제 트랜스크립션에서 찾아 자기 것으로 만드세요.</p>
      ${g.licks.map((l, i) => `
        <div class="card" style="padding:12px 14px">
          ...
          <div style="font-size:0.78rem;color:var(--text2);margin-top:4px">스케일 컨텍스트: ${l.scale}</div>
        </div>
      `).join('')}
    `;
