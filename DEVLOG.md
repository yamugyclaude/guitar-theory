...
## 2026-06-10 — 음악 엔진 전면 업그레이드 + 연습 탭 신설 + UI 리디자인

### 이론 엔진 재작성 (theory.js)
- 기능화성 분석: ii–V 단위 인식, 세컨더리 도미넌트(V7/x), 트라이톤 서브(subV7/x), 백도어(IVm7–bVII7–I), 모달 인터체인지(차용 소스 모드 명시), 경과 디미니시, 블루스 토널리티 감지(I7을 키로 인식)
- 풀 코드 파서: 슬래시 코드(C/E), 7b9·13#11·alt·ø·dim7·m(maj7)·6/9 전부
- 코드별 chord-scale 추천 (멜로딕 마이너 모드, HW/WH 디미니시, 비밥 포함) + 추천 이유 해설
- `spellInterval()` 음정 철자 엔진 — D의 3도=F#, Fm7의 b3=Ab 정확 표기 (export, voicing/practice 공유)
- AI 심층 해석: `deepHarmonyAnalysis()` (gemini-analysis.js) — 엔진 분석을 컨텍스트로 화성 서사/즉흥 전략/리하모/레퍼토리

### 보이싱 엔진 재작성 (voicing.js)
- 하드코딩 코드북 폐기 → Drop 2(3현세트×4인버전) / Drop 3(2세트×4) / 셸(R–7–3) 계산 생성
- 도수(R/3/5/7) 색상 라벨 다이어그램

### 연습 탭 신설 (practice.js, tab 10)
- 7개 모듈: 모달 인터체인지 / 리디안 도미넌트 / 얼터드 / 디미니시 / 멜로딕 마이너 시스템 / 쿼털 보이싱 / 비밥 어휘
- 12키 선택 → 연습 진행 코드명 자동 생성 + 특성음 강조 지판 다이어그램(12프렛) + 언제/어떻게 지침 + 연습 루틴

### 스타일 탭 재작성 (library.js)
- 잡지 피처 수준 6명: SRV(신규)/메이어/닉존스턴/거스리/새치/레티에리
- 섹션: 스타일 해부 / 시그니처 테크닉(연습법 포함) / 화성 어법 / 톤·장비 / 필수 청취(유튜브 검색 링크) / 연습 로드맵 / 보캐뷸러리
- 지어낸 TAB 스니펫 전부 제거 (analysis.js 포함)

### 기타
- 휴지통: 악보/차트 소프트 삭제(deleted/deletedAt) → 복구/영구삭제, 악보 탭 🗑 칩
- 레이아웃 3종 선택(설정): 사이드바/타이포/테이블 — gta_settings.layoutStyle
- Supabase 에코 루프 수정: 원격 수신 시 `window.__gtaApplyingRemote` 플래그로 auto-push 차단
- UI 리디자인(style.css): 다크 프로 토큰 정리(#0b0d10), 필 네비, 포커스 링, 카드 그림자, 모바일 하단바 헤더 누출 수정

### 점검 결과 (2026-06-10)
- 10개 탭 렌더+기능 테스트(보이싱 검색, 솔로 추천, 차트 에디터, 업로드 폼 토글, 테마 전환) 전부 통과, 콘솔 에러/경고 0
- 검증 케이스: Autumn Leaves(Gm) 기능분석, C 재즈 블루스(V7/IV·#IVdim7 구분), 트라이톤 서브/백도어, Drop2 교과서 폼 일치
